...
//		}
	...
	void send(const char *type, const char *content,std::string username);
	void clear();
	void BSONToChannel(mongo::BSONObj obj,unsigned long msgcount);
	std::vector<std::string> getMessage(long start_seq,int span);
};
